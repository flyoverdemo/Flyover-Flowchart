...
class CharacterPDFParser {
  ...
  async parsePDF(pdfPath, options = {}) {
    ...
    try {
      ...
      // Try to extract form field data first (for fillable PDFs)
      ...
      // Parse remaining sections (only if pages exist from text extraction)
      if (pages) {
  ...
  async processFormFields(fields, data) {
    ...
    for (const [fieldName, fieldObj] of Object.entries(fields)) {
      ...
      // ABILITY SCORES
        ...
        const score = parseInt(processedValue);
        if (!isNaN(score)) {
          data.abilities.strength.score = score;
          data.abilities.strength.modifier = Math.floor((score - 10) / 2);
        }
      } else if (lowerName === 'dex' || lowerName === 'dexterity') {
        const score = parseInt(processedValue);
        if (!isNaN(score)) {
          data.abilities.dexterity.score = score;
          data.abilities.dexterity.modifier = Math.floor((score - 10) / 2);
        }
      } else if (lowerName === 'con' || lowerName === 'constitution') {
        const score = parseInt(processedValue);
        if (!isNaN(score)) {
          data.abilities.constitution.score = score;
          data.abilities.constitution.modifier = Math.floor((score - 10) / 2);
        }
      } else if (lowerName === 'int' || lowerName === 'intelligence') {
        const score = parseInt(processedValue);
        if (!isNaN(score)) {
          data.abilities.intelligence.score = score;
          data.abilities.intelligence.modifier = Math.floor((score - 10) / 2);
        }
      } else if (lowerName === 'wis' || lowerName === 'wisdom') {
        const score = parseInt(processedValue);
        if (!isNaN(score)) {
          data.abilities.wisdom.score = score;
          data.abilities.wisdom.modifier = Math.floor((score - 10) / 2);
        }
      } else if (lowerName === 'cha' || lowerName === 'charisma') {
        const score = parseInt(processedValue);
        if (!isNaN(score)) {
          data.abilities.charisma.score = score;
          data.abilities.charisma.modifier = Math.floor((score - 10) / 2);
        }
      }
      // COMBAT STATS
  ...
  exportToJSON(data) {
    try {
      // Get next available temp character slot
      let tempCharIndex = 1;
      let slot = `char_${String(tempCharIndex).padStart(2, '0')}`;
      
      // Find next available slot
      while (localStorage.getItem(slot)) {
        tempCharIndex++;
        if (tempCharIndex > 99) {
          console.warn('Exceeded max temporary characters (99), overwriting char_01');
          tempCharIndex = 1;
          break;
        }
        slot = `char_${String(tempCharIndex).padStart(2, '0')}`;
      }

      // Store in localStorage
      const key = `char_${String(tempCharIndex).padStart(2, '0')}`;
      localStorage.setItem(key, JSON.stringify(data));
      
      // Keep track of all temp characters
      let tempList = JSON.parse(localStorage.getItem('temp_characters') || '[]');
      if (!tempList.includes(key)) {
        tempList.push(key);
        localStorage.setItem('temp_characters', JSON.stringify(tempList));
      }

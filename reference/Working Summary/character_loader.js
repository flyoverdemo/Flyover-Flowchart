...
class CharacterLoader {
  ...
  async loadCharacterData(path = null) {
    ...
    try {
      ...
      console.log('[CharacterLoader] Character data loaded:', this.characterData.character?.name);
    ...
    return new Promise((resolve, reject) => {
      ...
      reader.onload = (e) => {
        try {
          ...
          console.log('[CharacterLoader] Loaded from blob:', this.characterData.character?.name);
  ...
  getFromStorage() {
    try {
      ...
      if (json) {
        this.characterData = JSON.parse(json);
  ...
  applyActions() {
    ...
    // Aggregate all action types (primary display uses weapons first)
    const actions = [
